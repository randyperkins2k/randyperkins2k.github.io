...
*/
_.typeOf = function(valueOne) {
  if (Array.isArray(valueOne)) {
    return "array";
  } else if (valueOne === null) {
    return "null";
  } else return typeof valueOne;
};
...
*   _.first(["a", "b", "c"], 2) -> ["a", "b"]
*/
_.first = function(arrayOne, numberOne) {
  if (!Array.isArray(arrayOne) || numberOne < 0) {
    return [];
  }
  else if (numberOne === undefined || isNaN(numberOne)) {
    return arrayOne[0];
  }
  else return arrayOne.splice(0, numberOne);
};
...
*   _.last(["a", "b", "c"], 2) -> ["b", "c"]
*/
_.last = function(arrayOne, numberOne) {
 if (!Array.isArray(arrayOne) || numberOne < 0) {
    return [];
  }
  else if (numberOne === undefined || isNaN(numberOne)) {
    ...
    return arrayOne.splice(arrayOne.length - numberOne, numberOne);
  }    
};
...
_.filter = function(array, action) {
  var filtered = [];
  _.each(array, function(element, index, arrayOne) {
...
_.map = function (coll, action) {
  ...
  if (Array.isArray(coll)) {
    _.each(coll, function(element, index, arr) {
      newArray.push(action(element, index, arr));
    });
  }
  else if (typeof coll === 'object') {
    _.each(coll, function(element, index, object) {
      newArray.push(action(coll[index], index, coll));
    });
  } return newArray;
};
...
_.every = function(collection, action){
    ...
    //if the action applied on the element, index, array equals true,
    //push true into results array.
        } else if (action(elem, ind, arr) === true) {
            results.push(true);
    //if the action equals false, push false into results array.
        } else if (!action(elem, ind, arr)) {
            results.push(false);
        }
    });
    //if array contains no false elements, then return true. 
...
_.some = function(collection, action){
    ...
    //using contains, see if the results array contains at least one truthy value.
...
*/
_.reduce = function(collection, actionOne, seed) {
    var previousResult;
